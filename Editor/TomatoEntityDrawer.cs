...
namespace TomatoEntityDrawer
{
    ...
    public class TomatoEntityDrawer : Editor
    {
        ...
        private static bool _customClassFoldout;
        ...
        public static void DrawEntity(Entity entity)
        {
            ...
            if (_isValueSelected)
            {
                ...
                genericAddMethod.Invoke(_context, new object[] { entity });
            ...
            foreach (var componentType in _components)
            {
                ...
                if (hasComponent)
                {
                    ...
                    var componentRef =
                        genericGetComponentMethod.Invoke(_context, new object[] { entity });
                    ...
                    for (var i = 0; i < fields.Length; i++)
                    {
                        ...
                        if (GUI.changed)
                        {
                            ...
                            genericSetComponentMethod.Invoke(_context, new[] { entity, componentRef });
                        }
                    }
        ...
        private static void DrawField(FieldInfo fieldInfo, object component, object value)
        {
            ...
            {
                var editorValue = EditorGUILayout.Toggle(name, boolValue);
            ...
            {
                var tempInt = Convert.ToInt32(ulongValue);
                tempInt = EditorGUILayout.IntField(name, tempInt);
                fieldInfo.SetValue(component, (ulong)tempInt);
            }
            ...
            else if (valueType.IsSerializable)
            {
                ...
                _customClassFoldout = EditorGUILayout.Foldout(_customClassFoldout, $"{valueType.Name}");
                if (_customClassFoldout)
                {
                    var fields = value.GetType().GetFields(BindingFlags.Public | BindingFlags.Instance);
                    foreach (var field in fields)
                    {
                        var fieldValue = field.GetValue(value);
                        DrawField(field, value, fieldValue);
                    }
                }
        ...
        public static void SetColoredBoxStyle(GUIStyle coloredBoxStyle)
        {
            _coloredBoxStyle = coloredBoxStyle;
        }
    }
}